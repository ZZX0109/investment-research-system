...
> 当前事实源以 [`current-system.md`](current-system.md) 为准。本白皮书保留通用 PIT、研究评估和历史短周期任务说明；当前产品主线是 A 股长期投资 scorecard 与季度级 120/240 日研究，短周期方向/收益只作辅助诊断，所有产物继续保持 `research_only`。
...
系统仍保留四个彼此独立的短周期辅助诊断任务：未来 1 日方向三分类概率、未来 5 日方向三分类概率、未来 20 日收益 P10/P50/P90 区间，以及未来 20 日最大回撤分布与阈值回撤概率。长期主线另以经营质量、成长稳定性、估值位置、股东回报、行业/市场相对收益和长期回撤为研究对象。所有任务同时输出数据质量、覆盖率、模型版本、证据状态、模型分歧、拒答原因和 Shadow 前向验证状态；系统不输出买卖指令，不承诺收益，也不把风险概率解释为上涨或下跌概率。
...
正式路径需要 PostgreSQL、对象存储、授权主备数据源、SLA、历史 `available_at`、revision、证券状态、公司行动、PIT/leakage audit、独立审批、artifact hash、回滚产物和至少 60 个独立有效正式 Shadow 交易日。正式模型的发布单位是：
