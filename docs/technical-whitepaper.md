...
WorkBuddy 是面向金融投研的证据约束型 Agent。系统在 point-in-time 数据、模型不确定性与证据冲突条件下完成单资产未来 20 个交易日回撤风险研究，并在证据、特征、模型或引用不满足门禁时选择 repair、hold、block 或 abstain。它不执行交易，也不让语言模型计算收益率、回撤或风险概率。
